...
def generate_suggestions_endpoint():
    ...
    try:
        ...
        app.logger.info(
            f"定義済みのタグ {len(tag_definitions)} 件の類似度分析を開始します。"
        )
        generate_integration_suggestions(tag_definitions)

        return (
            jsonify(
                {"status": "success", "message": "最適化提案の生成が完了しました。"}
            ),
            200,
        )
        ...
        app.logger.error(
            f"提案生成中に予期せぬエラーが発生しました: {e}", exc_info=True
        )
...
def execute_integration_endpoint():
    ...
    try:
        ...
        suggestion_ref.update(
            {"status": "completed", "reviewedAt": firestore.SERVER_TIMESTAMP}
        )

        app.logger.info(f"タグ統合が完了しました: '{source_tag}' -> '{target_tag}'")
        return (
            jsonify({"status": "success", "message": "タグの統合が完了しました。"}),
            200,
        )

    except Exception as e:
        app.logger.error(
            f"統合処理中に予期せぬエラーが発生しました: {e}", exc_info=True
        )
        return jsonify({"status": "error", "message": "Internal Server Error"}), 500
...
def generate_definition_suggestions(undefined_tags):
    ...
    for tag in undefined_tags:
        ...
        existing_suggestions = (
            db.collection("suggestion_tags")
            .where("targetTag", "==", tag)
            .where("type", "==", "define_tag")
            .limit(1)
            .stream()
        )
        ...
        for j in range(i + 1, len(tags)):
            ...
            if tag1 in tag2 or tag2 in tag1:
                ...
                query = (
                    ...
                    .where("targetTag", "==", target)
                    .where("type", "==", "integrate_tags")
                )
...
def execute_tag_integration(source, target):
    ...
    for doc in articles_ref.stream():
        ...
        if source in data.get("aiGenerated", {}).get("categories", []):
            new_categories = list(
                set(
                    [
                        target if c == source else c
                        for c in data["aiGenerated"]["categories"]
                    ]
                )
            )
        ...
        if source in data.get("aiGenerated", {}).get("tags", []):
            new_tags = list(
                set([target if t == source else t for t in data["aiGenerated"]["tags"]])
            )
    ...
    for doc in dict_ref.stream():
        ...
        if source in data.get("constituentTags", []):
            new_constituents = list(
                set([target if t == source else t for t in data["constituentTags"]])
            )
